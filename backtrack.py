...
class HexagonalCalendarWithPiece:
    ...
    def solve_puzzle(self):
        ...
        # On définit l'ordre de placement des pièces (on parcourt les clés triées)
        pieces_order = sorted(self.pieces.keys())

        # Dictionnaire pour enregistrer le placement de chaque pièce :
        # {piece_id: (rotation_index, offset, positions_abs)}
        solution = {}
        ...
        if backtrack(0):
            ...
            self.solution = solution  # Stocker la solution comme attribut de classe
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        self.solve_puzzle()
                        # Vous pourriez vouloir afficher la solution visuellement
                        # en mettant à jour self.piece avec la première pièce de la solution
                        if hasattr(self, "solution") and self.solution:
                            self.current_piece = list(self.solution.keys())[0]
                            rotation_index, _, _ = self.solution[self.current_piece]
                            self.current_position = rotation_index
